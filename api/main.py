...
Receives WhatsApp messages via Twilio, classifies with Claude,
responds to customer, logs ticket to Supabase.
...
DISCO_KEYWORDS = {
    ...
    "Eko DisCo":            ["eko", "vi", "victoria island", "lagos island",
                             "surulere", "apapa"],
...
Customers may write in English, Pidgin English, or a mix of both.
Common Pidgin phrases: "NEPA don take light" = power outage, "light don go" = power outage,
"meter no dey work" = meter fault, "bill too much" = high bill complaint.
...
Respond ONLY with valid JSON — no markdown, no extra text:
{
  ...
  "response_message": "<empathetic WhatsApp reply in customer's language style, 2-4 sentences, include {TICKET_ID}>",
  "escalate": <true if safety risk, fire, sparking, or affects many customers; false otherwise>,
...
# ── Supabase ticket save ──────────────────────────────────────────
async def save_ticket(ticket: dict):
    if not SUPABASE_URL or not SUPABASE_KEY:
        logger.warning("Supabase not configured — ticket not saved")
        return
    async with httpx.AsyncClient() as client:
        ...
        if r.status_code in (200, 201):
            logger.success(f"Ticket {ticket['ticket_id']} saved")
        else:
...
async def whatsapp_webhook(request: Request, tasks: BackgroundTasks):
    """Twilio WhatsApp webhook — receives messages, classifies, responds."""
    ...
    if result.get("escalate"):
        logger.warning(
            f"ESCALATION [{ticket_id}]: {result.get('escalation_reason')}")
...
async def classify_direct(req: ClassifyRequest):
    """Test endpoint — classify a complaint without WhatsApp."""
    ...
    return {
        ...
        "response_preview":  result.get("response_message",
...
async def get_tickets(
    ...
    """Fetch tickets from Supabase for ops dashboard."""
    if not SUPABASE_URL or not SUPABASE_KEY:
